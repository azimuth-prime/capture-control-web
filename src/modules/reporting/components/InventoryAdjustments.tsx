...
export function InventoryAdjustments() {
  ...
  const { data, isFetching, isError, error, refetch } = useQuery({
    queryKey: ['inv-adjustments', filter],
    queryFn: () => inventoryReportService.findAdjustmentsByDates(filter!).then((r) => r.data),
    enabled: filter !== null,
  })
  ...
  return (
    <div className="p-4">
      ...
      <h1 className="mb-4 text-2xl font-bold">Inventory Adjustments</h1>
              ...
              <tbody className="divide-y">
                {data.map((row) => (
                  <tr key={row.id} className="hover:bg-muted/50">
                    ...
                    <td className="px-3 py-2">{row.adjustmentType ?? '—'}</td>
