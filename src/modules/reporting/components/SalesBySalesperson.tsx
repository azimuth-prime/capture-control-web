...
export function SalesBySalesperson() {
  ...
  const { data, isFetching, isError, error, refetch } = useQuery({
    queryKey: ['sales-by-salesperson', filter],
    queryFn: () => salesReportService.findOrdersBySalesperson(filter!).then((r) => r.data),
    enabled: filter !== null,
  })
  ...
  return (
    <div className="p-4">
      ...
      <h1 className="mb-4 text-2xl font-bold">Sales by Salesperson</h1>
        ...
        <>
          <div className="mb-2 flex items-center justify-between">
            <span className="text-sm text-muted-foreground">
              {data.length} salespersons &mdash; Total: ${grandTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
            <button
