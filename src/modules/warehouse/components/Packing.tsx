...
export function Packing() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['packable-orders', debouncedKeyword, page],
    queryFn: () =>
      warehouseService
        .findPackableOrderBykeyword({
    ...
    <div className='container-fluid px-4 py-2'>
      <nav className='mb-4 text-sm text-muted-foreground'>
        <ol className='flex gap-2'>
          ...
          <li>/</li>
          <li className='font-medium text-foreground'>Packing</li>
        </ol>
      </nav>
      ...
      <div className='mb-4'>
        <input
          type='text'
          placeholder='Search orders to pack...'
          value={keyword}
            ...
            <table className='w-full text-sm'>
              ...
              <tbody>
                {data.results.map((order) => (
                  <tr
                    key={order.id}
                    onClick={() => navigate(`/pack-details/${order.id}`)}
